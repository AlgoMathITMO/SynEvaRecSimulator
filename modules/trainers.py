...
def _rating_matrix_to_long_table(rating_matrix, drop_none = True) -> pd.DataFrame:
    df = pd.DataFrame(rating_matrix)
    df["user_id"] = df.index
    df_long = df.melt(id_vars=["user_id"], var_name="item_id", value_name="rating").copy()
    if drop_none:
...
class SvdTrainTestExecutor(evaluator.TrainTestExecutorABC):
    ...
    def __call__(self, rating_matrix: pd.DataFrame, test_size: float, sample_size: float) -> evaluator.TestSetError:
        ...
        dataset = surprise.Dataset.load_from_df(long_table[['user_id', 'item_id', 'rating']], surprise.Reader(rating_scale=(1, 10)))
...
class NmfTrainTestExecutor(evaluator.TrainTestExecutorABC):
    def __init__(self, config: t.Optional[dict] = None) -> None:
        self._config = config or {}
    ...
    def __call__(self, rating_matrix: pd.DataFrame, test_size: float, sample_size: float) -> evaluator.TestSetError:
        ...
        algo = surprise.NMF(**self._config)
        algo.fit(train_set)
        predictions = algo.test(test_set)
